...
const PlayerOfTheMatch = ({
  ...
  // Rezultati se prikazuju ako je korisnik glasao ILI ako je glasanje isteklo
  const shouldShowResults = hasVoted || isExpired;
  ...
  return (
    <div className="px-5 py-6 border-t border-white/05">
      ...
      <div className="flex items-center gap-2 mb-6">
        <span className="text-[24px]">{isExpired ? "📊" : "🏆"}</span>
        <div>
          <p className="text-[11px] font-semibold tracking-[0.15em] uppercase text-[#c49b32]">
            {isExpired
              ? t("vote.finalResults") || "Konačni rezultati"
              : t("vote.title")}
          </p>
          <p className="text-[12px] text-[#56544e]">
            {isExpired
              ? t("vote.closed") || "Glasanje je završeno"
              : hasVoted
                ? t("vote.thankYou")
                : t("vote.instructions")}
          </p>
      ...
      {/* Voting — Prikazuje se samo ako NIJE glasao i NIJE isteklo */}
      ...
      {/* Results — Prikazuje se nakon glasanja ili isteka */}
      {shouldShowResults && !loading && results.length > 0 && (
